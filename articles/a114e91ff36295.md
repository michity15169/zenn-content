...
検索結果は最終更新日などのカラムでソートしたい場合もありますが、文書検索などで検索キーワードの出現頻度など関連度が高い順で表示したい場合もあると思います。このような処理のためにSpannerでは[SCORE関数](https://cloud.google.com/spanner/docs/reference/standard-sql/search_functions#score)でランク付けをできます。この関数は概ね[TF-IDF](https://ja.wikipedia.org/wiki/Tf-idf)に基づき計算される値を返します。
