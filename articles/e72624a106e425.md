...
:::message
追記（2026年9月）
現在、Google Cloudコンソールの「独自のデータをインポート（Import my own data）」および`gcloud alpha spanner databases import`コマンドにより、外部ツールの導入やマニフェストファイル（`manifest.json`）の作成を行うことなく、より手軽にCSVファイルやMySQL・PostgreSQLのダンプファイルを取り込めるようになっています。
最新の手順については[「Cloud SpannerへCSVやダンプファイルを手軽にインポートする方法」](https://zenn.dev/google_cloud_jp/articles/d6e82c5a9f01b4)を参照してください。
なお、異種データベースからの移行性評価や詳細な型変換ルールを適用してインポートを行いたい場合には、本稿で紹介している`spanner-migration-tool`を直接実行する方法が引き続き有効です。
:::
