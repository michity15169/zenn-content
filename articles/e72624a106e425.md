...
:::message
**追記（2026年9月）**
現在、Google Cloudコンソールの**「Import my own data（独自データのインポート）」**および**`gcloud alpha spanner databases import`**コマンドにより、外部ツールの導入やマニフェストファイル（`manifest.json`）の作成を行うことなく、より手軽にCSVファイルやMySQL・PostgreSQLのダンプファイルを取り込めるようになっています。
