...
この並列書き込み時のレプリケーションおよび Paxos 合意コストを削減する機能が、Spanner 公式ドキュメントで「[スループットを最適化した書き込み](https://docs.cloud.google.com/spanner/docs/throughput-optimized-writes?hl=ja)（Throughput optimized writes）」として提供されている `MaxCommitDelay` オプションです。
