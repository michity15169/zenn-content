...
ノード数のサイジングに関しては、オートスケーラーを使うという方法もあります。[マネージドオートスケーラー](https://cloud.google.com/spanner/docs/managed-autoscaler?hl=ja)がEnterprise Edition以上で提供されています。
...
RDBではコネクション毎にデータベースのプロセスやスレッドが割り当てられているアーキテクチャを取られる事が多いですが、Spannerではコネクションを[フロントエンドで終端しています](https://cloud.google.com/spanner/docs/latency-points?hl=ja#spanner-api-requests)。そのため、同時コネクション数を大きくしてもコンピュートのリソースを過大に消費することはありません。RDBの感覚ではコネクションを集約するミドルウェアがサービス側に組み込まれているようなイメージです。
