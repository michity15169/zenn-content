...
1ノードは1000 PU（Processing Units/処理ユニット）とも呼ばれ、[1ノードの性能の目安が提供されています](https://cloud.google.com/spanner/docs/performance?hl=ja#typical-workloads)。ストレージは前述の通り自動的にスケールするため、サイジングのパラメータはノード数のみです。
