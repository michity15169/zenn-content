...
運用面で特に大きなメリットとなるのが完全オンラインでのスキーマ変更（DDL）です。一般的なRDBでは大規模テーブルに対する `ALTER TABLE`（カラム追加やインデックス生成）でテーブルロックが発生し、クエリ処理や書き込みの遅延・タイムアウトのリスクが生じますが、Spannerではダウンタイムやサービスへの影響なしにバックグラウンドで安全に実行されます。

### 高速で一貫性のある読み取り（Read-Only Transaction）

データの読み取り処理において、普通のRDBではロック競合や読み取り一貫性の制御（分離レベルの考慮）が必要になります。SpannerではTrueTime技術を活用したRead-Only Transaction（読み取り専用トランザクション）が提供されており、ロックを一切取得せずに過去の特定のタイムスタンプや最新の一貫したデータを高速に読み取ることが可能です。画面表示やレポート出力などの読み取り処理にRead-Only Transactionを明示的に使うことで、書き込み処理をブロックせず高いスループットを維持できます。
