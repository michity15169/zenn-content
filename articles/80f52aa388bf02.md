...
* **測定ワークロード**：
  * **Point Read (`Concurrency = 1`, 3,000 samples)**：実運用において単一行の参照（Point Read）を行う際は、Paxos リーダーへのリース確認によるオーバーヘッドを避けて近傍レプリカから高速に読み取るため、Stale Read を用いるのが一般的です。そこで本計測では `client.Single().WithTimestampBound(spanner.ExactStaleness(15 * time.Second)).ReadRow` による 1 行読み取り（1 リクエスト＝**1 RPC 往復**）を実施しました。
...
1. **ボリュームゾーン（山のピーク）の左シフトと、マルチゾーン構成特有の「2 つの山」**
   左上パネル（Point Read c=1 の度数分布ヒストグラム）を見ると、Direct Connectivity（青）は GFE 経由（グレー）に対して分布の山全体が左へ約 0.8〜0.9 ms そのまま移動しています。中央値（p50）は `2.68 ms` から **`1.84 ms`（-0.83 ms / 31.1% 短縮）** へと改善し、直列実行（`c=1`）でも最小値（Min）が **`0.98 ms`（1 ミリ秒未満）** に到達しました。
   また、Paxos リーダー確認を行わない Stale Read であっても、ヒストグラムの形状を見ると **`1.4〜1.6 ms` 付近の大きな山（第 1 ピーク：全体の約 2/3）** と、**`2.8〜3.0 ms` 付近の小さな山（第 2 ピーク：全体の約 1/3）** の 2 峰性（バイモーダル分布）になっていることが分かります（GFE 経由でも `2.3 ms` 付近と `3.6 ms` 付近に 2 つの山が存在します）。これは東京リージョン（`regional-asia-northeast1`）を構成する 3 つのゾーン（`a` / `b` / `c`）間の物理ネットワーク距離（ゾーン間 RTT）に起因しています。クライアント VM（ゾーン `b`）から近傍の 2 ゾーン（同一ゾーン含む）のレプリカで処理された通信（約 67%）が第 1 ピークを形成し、物理距離が離れた残り 1 ゾーンを経由した通信（約 33%）がゾーン間往復遅延（約 `+1.35 ms`）を伴って第 2 ピークを形成しています。GFE バイパスによって、同一ゾーン・別ゾーンの双方で等しく約 0.8〜0.9 ms のオーバーヘッドが削減されていることが確認できます。
2. **累積分布（CDF）とテールレイテンシー（p99〜p99.9）の改善**
   左下パネル（CDF 曲線）を見ると、Go SDK のデフォルトである 4 本の gRPC チャネルを並行稼働させた Concurrency = 4（`c=4 Direct Connectivity`、青の一点鎖線）では、すべてのチャネルが常時アクティブに保たれることで p50 が `2.56 ms` から **`1.64 ms`（-0.92 ms / 36.0% 短縮）** に達し、最小値（Min）は **`0.80 ms`** を記録しています。
   右下パネル（パーセンタイル棒グラフ）でも、Point Read (c=1) の p99 は `5.99 ms` から **`3.68 ms`（-2.31 ms / 38.6% 削減）**、p99.9 は `8.33 ms` から **`5.13 ms`（-3.20 ms / 38.4% 削減）** へと改善しており、分布の裾（テール）まで一貫して約 4 割の短縮効果が得られています。
...
* **レイテンシー改善効果**：東京リージョン 1,000 PU（1 Node）環境・Go クライアントの実測において、Point Read（Stale Read `ExactStaleness=15s`）の中央値（p50）が **`2.68 ms → 1.84 ms`（c=1 で -31.1%）**、4 並列（c=4）では **`2.56 ms → 1.64 ms`（-36.0%）** に短縮され、最小値は **`0.80 ms`（c=1 でも `0.98 ms` と 1 ミリ秒未満）** に達しました。テールレイテンシーについても p99 で **`5.99 ms → 3.68 ms`（c=1 で -38.6% 削減、c=4 では `7.54 ms → 3.85 ms` と -49.0% 削減）** とプロキシ通過時のジッターが大きく抑えられています。
