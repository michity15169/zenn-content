...
本記事では、Direct Connectivity によって通信経路がどのように変化するかを解説した上で、東京リージョン（`asia-northeast1`）の **1,000 Processing Units（1 Node）環境** と Go SDK（`cloud.google.com/go/spanner` v1.95.1）を用いて低並列度で測定したレイテンシーの変化（度数分布ヒストグラムやパーセンタイル比較）を紹介します。あわせて、利用する上での留意点である「クライアントのメモリ消費量の増加」がなぜ起きるのかを、Go ドライバの実装コードと測定結果から確認します。
...
測定した 3 つのパターン（Point Read 1並列 / Point Read 4並列 / Read-Write トランザクション 1並列）について、レイテンシーごとのリクエスト件数を表す**度数分布（ヒストグラム）**と、全体の割合を表す**累積分布（CDF）・パーセンタイル比較**の 2 つのグラフに分けて示します。
...
#### 2. 累積分布（CDF）と主要パーセンタイルの比較（全 3 パターン）

![累積分布（CDF）とパーセンタイルの比較](/images/spanner_direct_connectivity_cdf_percentiles.png)

* 左側の **CDF（累積分布関数）** では、曲線が左上に寄るほど短い時間で処理が完了していることを示します。3 つのパターンすべてにおいて、Direct Connectivity の実線・一点鎖線が GFE 経由の破線よりも左側に位置しています。
* 右側の **パーセンタイル棒グラフ** を見ると、中央値（p50）だけでなく **p99（上位 1% の遅延）** でも改善が確認できます。特に Point Read では、1 並列（c=1）の p99 が `5.99 ms → 3.68 ms`（**-38.6%**）、4 並列（c=4）の p99 が `7.54 ms → 3.85 ms`（**-49.0%**）と、プロキシ経由で発生していた遅延のばらつきが抑えられています。

> **※補足**：これらの測定値は手元の検証環境で試行した際の一例（参考情報）であり、クライアントの配置ゾーンやネットワーク状況、時間帯、データベースの負荷状況によって数値は変動します。
...
主要な要因は、Direct Connectivity の経路に万が一問題が生じた際、リクエストを失敗させずに従来の GFE 経由へ即座に切り替えられるよう、クライアント内部で **フォールバック用のコネクションプールもあわせて保持する実装** になっていることです。
...
* **レイテンシー改善効果（検証環境での参考値）**：手元の東京リージョン 1,000 PU（1 Node）環境・Go クライアントでの試行において、Point Read（Stale Read `ExactStaleness=15s`）の中央値（p50）が **`2.68 ms → 1.84 ms`（c=1 で -31.1%）**、4 並列（c=4）では **`2.56 ms → 1.64 ms`（-36.0%）** に短縮され、最小値は **`0.80 ms`（c=1 でも `0.98 ms` と 1 ミリ秒未満）** となりました。テールレイテンシーについても p99 で **`5.99 ms → 3.68 ms`（c=1 で -38.6% 削減、c=4 では `7.54 ms → 3.85 ms` と -49.0% 削減）** とプロキシ通過時のばらつきが抑えられました。
...
* **メモリ消費量の留意点**：障害時に即座に GFE 経由へ退避するためのフォールバック用コネクションプールを保持すること、およびエンドポイント管理データ構造をクライアント側で保持することにより、生存ヒープが約 +3.44 MB 増加します。これが Go ランタイムの GC Target（`HeapSys`）の予約によって約 2 倍の領域として確保され、今回の検証環境では **1 クライアントあたり約 7.5〜9.2 MB のメモリ増加（参考値）** として観測されました。`spanner.Client` のシングルトン再利用を行っていれば実用上の支障になることは少なく、環境変数 1 つ（`GOOGLE_SPANNER_ENABLE_DIRECT_ACCESS=true`）で導入できる有用な機能です。
