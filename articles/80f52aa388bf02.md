...
Spanner は分散データベースという特性のため多くのコンポーネントで構成されています。そのため一般的な RDB に比べると単純な処理を行ったときでもレイテンシーが高くなる傾向にありました。レイテンシーの原因の一つは Spanner へのアクセスが **Google Front End（GFE）** を経由することに由来します。これに対し、GFE をバイパスしてクライアント VM から Spanner の API フロントエンドへ直接 gRPC 通信を行う機能が [**Direct Connectivity**](https://docs.cloud.google.com/spanner/docs/latency-points#direct_connectivity) です。

この記事では、Direct Connectivity によって通信経路がどのように変わるのかを説明し、東京リージョンの Spanner インスタンスと Go SDK（`cloud.google.com/go/spanner` v1.95.1）を用いて測定したレイテンシーの変化（度数分布ヒストグラムやパーセンタイル比較）を紹介します。あわせて、利用する上での留意点である「クライアントのメモリ消費量の増加」がなぜ起きるのかを、Go ドライバーの実装コードと測定結果から確認します。

> **※注記**：本記事に掲載しているレイテンシーやメモリ消費量の測定値は、あくまで手元の検証環境で試行した際の参考情報であり、実際の環境やワークロードによって数値は異なります。

---

## Direct Connectivityの仕組み
...
一方、Direct Connectivity を有効にすると、GCE / GKE 上のクライアントは Google Cloud の内部ネットワークを介して、Spanner API フロントエンドへ **GFE を経由せずに直接 gRPC 通信** を行います。
...
flowchart LR
    ...
    subgraph Direct ["Direct Connectivity"]
        ...
        App2 -.->|"1. エンドポイント情報の取得"| TD
        App2 ==>|"2. GFE をバイパスして直接通信"| Spanner2
    end
```

### ルート解決とdp_checkによる事前確認
...
要件を満たしていれば、以下のように各項目のチェックが `PASSED` と出力されます。
...
測定した 3 つのパターンについて、レイテンシーごとのリクエスト件数を表す**度数分布（ヒストグラム）**と、中央値や上位 1% の遅延を表す**主要パーセンタイル（p50 / p90 / p99）の棒グラフ**を示します。
...
ヒストグラムを見ると、3 パターンとも Direct Connectivity では分布全体がそのまま左（低レイテンシー側）へ 0.8〜1.0 ms ほどシフトしています。

また、パーセンタイルの棒グラフ（黄色破線枠の `p99`）を見ると、中央値だけでなく上位 1% のテールレイテンシーの跳ね上がりが大きく抑えられていることがわかります。特に 4 並列（c=4）の Point Read では GFE 経由の p99 が `7.54 ms` まで伸びていたのに対し、Direct Connectivity では `3.85 ms` と約半分に収まっています。
...
5 クライアント作成時に 1 クライアントあたりの増分が小さくなっているのは、`grpc-go` の xDS クライアント実装や Traffic Director との通信キャッシュの一部がプロセス内で共有されるためです。

通常の GCE VM や GKE Pod であれば 10 MB 前後のメモリ増加が問題になることは少ないですが、リクエストごとに `spanner.NewClient` を作り直すような実装をしている場合や、コンテナのメモリ上限を 128〜256 MB 程度に切り詰めている環境では注意が必要です。原則通り `spanner.Client` はアプリケーション起動時に 1 度だけ初期化して使い回し、コンテナのメモリ上限には 15〜20 MB ほどの余裕を持たせておくと安全です。
...
Direct Connectivity は、環境変数 `GOOGLE_SPANNER_ENABLE_DIRECT_ACCESS=true` を設定するだけで GFE プロキシのオーバーヘッドをバイパスし、Spanner へのアクセスレイテンシー（特に p99 のテールレイテンシー）を下げられる機能です。

クライアント側でフォールバック用チャネルや xDS のルート情報を保持する分だけ 1 クライアントあたり 10 MB 弱のメモリ増加を伴いますが、GCE / GKE 上で Go・Java アプリケーションを運用している場合は手軽に試せる改善策になります。
