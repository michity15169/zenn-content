...
![ビットリバースの様子](/images/spanner_bit_reverse.png)
...
![ビットリバースの結果がどのように分布するかのアニメーション](/images/bit_reverse_animation.gif)

ビットリバース関数に1から順番に100までの数値を与えたときに結果がどのような分布となるかを示したアニメーションです。左は縦軸が1から100まで、右のグラフは縦軸がINT64の値が取り得る範囲であることに注意してください。関数にシーケンシャルな値を入力しても、結果がバラバラと分布することが分かると思います。この性質によって、Spannerの特定のノードだけが忙しいという状況が回避できます。
